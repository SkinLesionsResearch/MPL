...
def get_cifar10(args):  # 预处理：1.翻转 2.裁剪 3.标准化
    transform_labeled = transforms.Compose([  # Compose 容器，把四个操作打包成一个操作
        transforms.RandomHorizontalFlip(),  # 作随机水平翻转
        transforms.RandomCrop(size=args.resize,  # 作随机裁剪
...
def x_u_split(args, labels):
    ...
    for i in range(args.num_classes):
        ...
        cur_class_size = len(idx)
        label_per_class = args.num_labeled // args.num_classes
        perc_labeled = args.perc_labeled
        label_per_class = int(perc_labeled * cur_class_size)
...
class TransformMPL(object):   # 无标签图片增强
